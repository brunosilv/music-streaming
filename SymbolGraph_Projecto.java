 ...
 *  @author Kevin Wayne
 */
public class SymbolGraph_Projecto {
    ...
    /**
     * Custom AED2 - Constructor
     * @param n 
     */
    public SymbolGraph_Projecto(int n) {
     ...
     */
    public SymbolGraph_Projecto(String filename, String delimiter) {
    ...
    public void addEdge(String s1, String s2) throws Exception {
        if(!contains(s1) || !contains(s2)) {
            throw new Exception("SymbolGraph_AED2(): One of the params is not in ST, add it first with putSymbol()");
        }
        ...
        if(G_counter > G.V()) {
            Graph g_npointer = new Graph(st.size());
            for (int v = 0; v < G.V(); v++) {
                for (int w : G.adj(v)) {
                    g_npointer.addEdge(v, w);
                }
            }

            // novo Edge
            g_npointer.addEdge(i1, i2);
            G = g_npointer;
        }
        else {
            G.addEdge(i1, i2);
